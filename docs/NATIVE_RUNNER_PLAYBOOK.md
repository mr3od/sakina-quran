...
| 9 | Valid renders flagged blank / garbage published as evidence — **or CI green while the app is dead** | Size-threshold asserts are content-blind: chrome-less pages compress tiny; dialogs photograph fine. Worse: `am start -W` prints `Status: ok` the moment the activity exists — a JS crash one second later still screenshots as the launcher at full size, passing every size/count assert (hit for real: uniwind boot NPE, SDK 57) | Calibrate threshold to actual output (8 KB worked here), assert count too; dump `adb logcat -b crash` after launches and fail the job when it is non-empty (see native-runner-evidence.yml); **always read the screenshots** — the crash-buffer assert catches only crash-class wrong content, not a wrong page |
